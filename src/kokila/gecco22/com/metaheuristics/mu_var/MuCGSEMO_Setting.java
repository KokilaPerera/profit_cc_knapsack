 ...
 **/
public class
MuCGSEMO_Setting extends ExperimentSetting {
    ...
    public void init(Knapsack prob, double delta, int iterations) throws JMException {
        ...
        problem = new MuC_KnapsackMO( prob.getName());
        ...
        algorithm = new GSEMO(problem) {
            ...
            public boolean weaklyDominates(Solution first, Solution second) {
                ...
                double secondMean = second.getObjective(MuC_KnapsackMO.MU);
                double firstVar = first.getObjective(MuC_KnapsackMO.Csqrt);
                double secondVar = second.getObjective(MuC_KnapsackMO.Csqrt);
            ...
            public SolutionSet sanitize(SolutionSet population)
            {
                ...
                population.sort(new ObjectiveComparator(MuC_KnapsackMO.MU,true));
                Solution s1 = population.get(0);
                double maxCheby = s1.getObjective(MuC_KnapsackMO.MU)- Math.sqrt(temp * s1.getObjective(MuC_KnapsackMO.Csqrt));

                newPopulation.add(s1);
                for(int i=1; i<population.size();i++)
                {
                    Solution s2 = population.get(0);
                    double cheby = s2.getObjective(MuC_KnapsackMO.MU)- Math.sqrt(temp * s2.getObjective(MuC_KnapsackMO.Csqrt));
                    if(cheby > maxCheby)
                    {
    ...
    protected void logResults(SolutionSet population){
        ...
        for (Iterator<Solution> it = population.iterator(); it.hasNext(); ) {
            ...
            for(int i = 0; i<alphas.length;i++)
            {
                ...
                double mu = solution.getObjective(MuC_KnapsackMO.MU);
                double var = solution.getObjective(MuC_KnapsackMO.Csqrt);
                int count = (int)(var*var);
                var = count * problem.getV();//* delta * delta / 3.0;
